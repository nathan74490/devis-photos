 ...
 *   {
 *     "format_code": "A4",
 *     "support_code": "papier_bril",     // optionnel
 *     "finish_code": "vernis",           // optionnel
 *     "qty": 10,
 *     "vat_rate": 20,
...
router.post("/:quoteId/items", async (req, res) => {
  ...
  try {
    ...
    // 2) Calcul prix (format + support + finition)
    ...
    if (!price.ok) {
      await client.query("ROLLBACK");
      return res.status(400).json(price); // renvoie l'erreur (ex: FORMAT_REQUIRED)
    }

    // 3) Résoudre les IDs depuis codes si besoin
    ...
    // 4) INSERT dans quote_items (sans setup_fee_ex_vat)
    const breakdown = {
    ...
    // 5) Recalcul des totaux du devis (HT/TVA/TTC) avec le même taux de TVA
